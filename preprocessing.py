...
def _resize_for_ocr(
    image: np.ndarray,
    target: int = OCR_TARGET_LONG_SIDE,
) -> np.ndarray:
    height, width = image.shape[:2]
...
def preprocess_version_a(original: np.ndarray) -> np.ndarray:
    ...
    corrected = _rotate_bound(
        corrected,
        estimate_rotation(gray),
    )
...
def segment_long_receipt(
    ...
    if (
        height / max(1, width) < OCR_LONG_RECEIPT_RATIO
        or height <= OCR_TARGET_LONG_SIDE
    ):
